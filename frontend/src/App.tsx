...
type Passport = { season:{name:string}; count:number; stamps:any[]; rewards:any[] };
function App(){
  const [auth,setAuth]=useState<TwitchAuth>(); const [data,setData]=useState<Passport>(); const [error,setError]=useState('');
  async function load(a=auth){ if(!a) return; try{ setData(await api('/api/passport',a)); }catch(e:any){setError(e.message)} }
  async function checkin(){ if(!auth) return; await api('/api/checkin',auth,{method:'POST'}); await load(auth); }
